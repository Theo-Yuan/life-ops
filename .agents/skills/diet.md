---
description: "训记饮食数据 Open API —— 查询/搜索/写回饮食记录、自定义食物、模板"
---
...
@_shared/auth.diet.md

> **用户画像**: 另见 `projects/workout/.agents/profile.md`（agent 需用 Read 工具单独加载）
...
Authorization: Bearer {{XUNJI_DIET_API_KEY}}
...
### 饮食数据特有
- 饮食记录接口同一用户同类接口 15 秒一次；食物搜索接口同样 15 秒一次。
